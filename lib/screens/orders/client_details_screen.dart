...
class _ClientDetailsScreenState extends State<ClientDetailsScreen> {
    ...
    try {
      ...
      if (orders.isNotEmpty) {
        ...
        _materialsController.text =
            (primary['special_requirements'] as String?) ?? '';
      }
      if (mounted) {
        setState(() {
          _orders = orders;
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Failed to load client: $e')));
      }
    }
  }
  ...
  void _addAnotherIdol(BuildContext context) async {
    ...
    showDialog(
      context: context,
      builder: (BuildContext context) {
        final TextEditingController idolNameController =
                ...
                onPressed: () async {
                  if (idolNameController.text.isNotEmpty) {
                    try {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text('Idol added successfully'),
                        ),
                      );
                    ...
                    if (_orders.isEmpty)
                      ...
                      for (final order in _orders) ...[
                        _buildDeliveryDateCard(
                          (order['idol_name'] as String?)?.trim().isNotEmpty ==
                                  true
