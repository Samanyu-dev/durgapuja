...
class OpenAISpeechService {
  ...
  Future<String> translateToEnglish(File audioFile) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        LoggingService.logDebug(
          'OpenAI Translation API error: ${response.statusCode}',
        );
        LoggingService.logDebug('Response: ${response.body}');
  ...
  Future<Map<String, String>> transcribeAndTranslate(
    ...
    try {
      ...
      return {'bengali': bengaliText, 'english': englishText};
    } catch (e) {
          ...
          'messages': [
            {
              'role': 'system',
              'content':
                  'You are a professional Bengali to English translator. Translate the following Bengali text to English accurately.',
            },
